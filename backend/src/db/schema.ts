...
export const certificateStatusEnum = pgEnum("certificate_status", ["generating", "pending", "ready", "failed"]);
export const bulkJobStatusEnum = pgEnum("bulk_job_status", ["pending", "processing", "completed", "failed"]);
...
export const bulkJobs = pgTable("bulk_jobs", {
    id: text("id").primaryKey(),
    eventId: text("event_id").notNull().references(() => trainingEvents.id, { onDelete: "cascade" }),
    ...
    eventIdIdx: index("bulk_jobs_event_id_idx").on(table.eventId),
